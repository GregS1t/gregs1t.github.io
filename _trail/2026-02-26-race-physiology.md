...
Group by slope bin, compute medians (robust to outliers). This analysis builds your **individual cost curve**, directly comparable to the Minetti polynomial.

The key: we compare your **raw pace** (not GAP) with the Minetti prediction. The model predicts how raw pace should vary with slope for a given flat pace. If the curves are close, the model fits you well. Where they diverge, you are either more or less efficient than the model's sample.

```python
SLOPE_BINS = [-30, -15, -10, -7, -5, -3, -1, 1, 3, 5, 7, 10, 15, 30]

def slope_crossanalysis(df, bins=SLOPE_BINS):
    """Aggregate raw pace, GAP, and HR by slope bin (median)."""
    df = df.copy()
    df["slope_bin"] = pd.cut(df["slope_pct"], bins=bins)
    cols = ["slope_pct", "pace_s_per_km", "gap_s_per_km"]
...
The comparison between your actual curve (orange) and the Minetti prediction (blue dashed) is directly interpretable. If you are systematically above Minetti on descents, you are slower than the model predicts, meaning you brake more. If you are below on climbs, your uphill economy is better than the model's sample, or you pushed harder than a sustainable cruise effort.
