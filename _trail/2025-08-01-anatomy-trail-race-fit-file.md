...
GARMIN/SUUNTO/COROS ARE WATCHING YOU ! Yes, your watch knows everything about your run (and more). The exact moment you started climbing, the second your heart rate spiked, the GPS coordinates of every step you took. It recorded all of it, faithfully, for hours.
...
This is the first article in a series on trail running data analysis. No black box, no app doing the thinking for you. Just basic Python, your own data, and a bit of signal processing. By the end of this series, you will be able to detect fatigue, model training load, and extract physiological insights from a simple GPS file.

> This post is for anyone who’s curious about how to use their data
...
What is actually available depends on your device. A basic GPS watch gives you coordinates, distance, and speed. Add a heart rate strap and you get cardiac data on top. A footpod brings cadence and stride length into the picture. Some fields will be missing depending on your setup, and that is fine.
...
To install the useful libraries, one can use `pip`, `conda` or whatever (Anaconda GUI installer...). I’m assuming you have some knowledge of Python...
...
- First, altitude often appears twice: `altitude` and `enhanced_altitude`. The enhanced version uses **barometric correction** when available and is generally more accurate on climbs. Same story for speed. We will always prefer the enhanced version when it exists.
...
It sounds harmless. But when you later compute slope as the ratio of altitude change over distance change, a negative distance increment produces a nonsensical spike. Better to fix it early. It's fairly easy: 
...
Now that we have walked through each step individually, we can put them together into a single cleaining function.
...
> **Disclaimer:** I am a research engineer, not a sports physiologist. What you read here is the notebook of a curious trail runner who likes understanding (his) data, not medical or training advice, not my job. The analyses are provided for informational purposes only. Sources and scientific references are there so you can check for yourself.
