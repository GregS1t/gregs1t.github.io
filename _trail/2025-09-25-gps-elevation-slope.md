...
We have to fix this mess ! Actually, the fix is to compute slope over a **backward distance window** : for each point, we find the point roughly `WINDOW_M` meters behind on the track and compute the slope between the two. The result is a real local gradient, stable and usable.
...
The choice of `WINDOW_M` is a tradeoff. Too small (< 30 m) and the slope stays noisy on basic GPS recordings. Too large (> 200 m) and you smooth out real transitions. 100 m is a reasonable starting point. If your watch has a barometric altimeter, 50 m is often enough. After all, try different values.
...
<div class="plotly-container">
  <iframe src="{{ site.baseurl }}/assets/img/blog/2025-09-gps-elevation/segmentation.html"
...
```
<div class="plotly-container">
  <iframe src="{{ site.baseurl }}/assets/img/blog/2025-09-gps-elevation/walk_probability.html"
          width="100%" height="350" frameborder="0" scrolling="no">
  </iframe>
</div>
