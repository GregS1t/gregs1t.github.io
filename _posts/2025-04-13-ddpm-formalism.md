...
L'entraînement optimal consisterait à maximiser la log-vraisemblance $\log p_\theta(\mathbf{x}_0)$ c'st à dire la probabilité d'observer les vraies images sous le modèle. Ce calcul est insoluble directement, mais on peut dériver une **borne inférieure variationnelle** (ELBO) qui décompose l'objectif en termes interprétables.
...
Le U-Net joue le rôle de $\boldsymbol{\epsilon}_\theta$ et il prédit le bruit qui a été ajouté à $\mathbf{x}_0$ pour obtenir $\mathbf{x}_t$.
...
C'est une **MSE entre le bruit réel et le bruit prédit**. Elle est directement implémentée dans `compute_loss` :
